...
public class DtoResidenceCreateRequest extends DtoPublicRequest{
	
	@ApiDescription("ID ,不填时为创建，否则为更新")
	@ApiDemoData("12101")
    private Integer id;

	@ApiDescription("申请人姓名")
	@ApiDemoData("")
	@Length(min=1,max=20,message="名称长度在1-20个字之间")
    private String name;

	@ApiDescription("申请人身份证")
	@ApiDemoData("")
	@Pattern(regexp="(\\d{14}[0-9a-zA-Z])|(\\d{17}[0-9a-zA-Z])",message="身份证格式不正确")
    private String idcard;

	@ApiDescription("申请人电话")
	@ApiDemoData("")
	@Length(min=8,max=11,message="电话号码为8-11位数字")
    private String cellphone;

	@ApiDescription("申请类型,查数据字典接口,落户申报类型:residenceType")
	@ApiDemoData("1")
    private Integer catelog;

	@ApiDescription("登记表照片 多个逗号分隔 ")
	@ApiDemoData("")
    private String imgsRegister;

	@ApiDescription("身份证照片 多个逗号分隔 ")
	@ApiDemoData("")
    private String imgsIdcard;

	@ApiDescription("户口薄照片 多个逗号分隔 ")
	@ApiDemoData("")
    private String imgsResidence;

	@ApiDescription("房权证照片 多个逗号分隔 ")
	@ApiDemoData("")
    private String imgsHouse;

	@ApiDescription("劳动合同照片 多个逗号分隔 ")
	@ApiDemoData("")
    private String imgsJob;

	@ApiDescription("社会保险照片 多个逗号分隔 ")
	@ApiDemoData("")
    private String imgsInsurance;

	@ApiDescription("营业执照照片 多个逗号分隔 ")
	@ApiDemoData("")
    private String imgsLicence;

	@ApiDescription("随迁人员照片 多个逗号分隔 ")
	@ApiDemoData("")
    private String imgsFamily;

	@ApiDescription("状态：0 保存，1 提交 ")
	@ApiDemoData("1")
