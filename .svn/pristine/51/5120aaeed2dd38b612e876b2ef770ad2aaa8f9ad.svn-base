...
public final class HttpClientUtils {
	...
	public static final String httpPostBody(String url, Map<String, String> params, Map<String, String> headers, String body){
		HttpClient client = new HttpClient();
		...
		try {
			...
			if(isValidHttpCode(code)){
				...
				log.error("Http response code(" + code + ") is not 200 when post " + url + " with parameters " + params+" body "+body);
			}
		} catch (HttpException e) {
			log.error("Unexpected HttpException when http post " + url + " with parameters " + params+" body "+body, e);
		} catch (IOException e) {
			log.error("Unexpected IOException when http post " + url + " with parameters " + params+" body "+body, e);
		} finally{
			RequestCounter.instance.count(url, code, (int)(System.currentTimeMillis() - timestamp));
		}
		return null;
